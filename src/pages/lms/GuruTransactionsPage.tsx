            ...
            {data.map((r) => {
              ...
              <tr key={r.id} className="border-b last:border-0">
                <td className="py-4 px-4">
                  ...
                  {(r.quantity ?? 1) > 1 && (
                    <p className="text-xs text-gray-500 mt-0.5">x {r.quantity} siswa</p>
                  )}
                </td>
                <td className="py-4 px-4 text-gray-600">{formatDate(r.paidAt)}</td>
                <td className="py-4 px-4 font-medium">
                  <p>{formatRupiah(r.total)}</p>
                  {(r.quantity ?? 1) > 1 && (
                    <p className="text-xs text-gray-500 mt-0.5">
                      {r.unitPrice ? `${formatRupiah(r.unitPrice)} x ${r.quantity}` : 'Pembelian kolektif'}
                      {r.uniqueCode ? ` + kode unik ${r.uniqueCode}` : ''}
                    </p>
                  )}
                </td>
                <td className="py-4 px-4">
                ...
                <td className="py-4 px-4">
                  {r.orderId ? (
                    <div className="flex flex-col gap-1">
                      ...
                      {canUploadProof && (
                        <a href={`#/guru/transactions/confirm?order=${encodeURIComponent(r.orderId)}`} className="text-amber-700 font-medium hover:underline text-xs sm:text-sm">
                          Upload bukti
                        </a>
                      )}
                    </div>
