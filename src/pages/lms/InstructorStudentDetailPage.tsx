              ...
              {tryoutRows.length === 0 ? (
                ...
                tryoutRows.map((row) => (
                  <tr key={row.attemptId} className="border-b last:border-0">
                    ...
                    <td className="py-3 px-4">
                      <a
                        href={`#/instructor/tryouts/${encodeURIComponent(row.tryoutId)}/attempts/${encodeURIComponent(row.attemptId)}/ai-analysis?redirect=${encodeURIComponent(`#/instructor/students/${studentId}`)}`}
